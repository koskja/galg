...
impl SubsetCollection<0, f32> for f32 {
    ...
    fn assign(&mut self, elem: f32, _: &impl IndexSubset<0>) -> Option<f32> {
        let r = Some(*self);
        *self = elem;
        r
    }

    fn project(&self, _: &impl IndexSubset<0>) -> Option<f32> {
        Some(*self)
    }
    ...
    for PlusAlgebra<DIM, EN2, A>
{
    ...
    fn assign(&mut self, elem: f32, i: &impl IndexSubset<{ DIM + 1 }>) -> Option<f32> {
    ...
    fn project(&self, i: &impl IndexSubset<{ DIM + 1 }>) -> Option<f32> {
        let bin = Subbin::convert_from(i).to_bits();
        let lower_bits = DIM;
        let mask = usize::MAX << lower_bits; // lowest significant one overlaps precisely the added vector
        if bin & mask != 0 {
    ...
    fn iter(&self) -> impl Iterator<Item = (f32, Self::Index)> {
        (0..(0x2 << DIM))
            .map(|i| Subbin::bits(i))
            .flat_map(|i| self.project(&i).map(|j| (j, i)))
    }
}
