...
use galg::{
    ...
    variable::Expr,
    AnyCliff, CliffAlgebra, G2Var, G3Var, G4Var, M4Var, G2, G3, M1, M4,
};
...
fn main() {
    ...
    return;
    let proj = CelPer::new(
        Vector3::new(Expr::nconst(1.), Expr::zero(), Expr::zero()),
        Expr::nconst(0.3),
    );
    let l_phi = Expr::nvar("lp");
    let p_phi = Expr::nvar("pp");
    let l: MobiusMap<2, Expr, G2Var> = MobiusMap::lorentz(l_phi);
    let p: MobiusMap<2, Expr, G2Var> =
        MobiusMap::parabolic(Vector2::new(Expr::nconst(1.), Expr::zero()), p_phi);
    ...
    println!(
        "{}",
        js_transformation("transform", |a: SVector<Expr, 5>| {
            let l: MobiusMap<2, Expr, G2Var> = MobiusMap::lorentz(a[3]);
            let p: MobiusMap<2, Expr, G2Var> =
                MobiusMap::parabolic(Vector2::new(Expr::nconst(1.), Expr::zero()), a[4]);
    ...
    println!(
        "{}",
        js_transformation("depth", |a: SVector<Expr, 3>| {
            Vector1::new(proj.dist(a))
        })
    );
    println!(
        "{}",
        js_transformation("project", |a: SVector<Expr, 3>| { proj.apply(a) })
    );
}

fn js_transformation<const N: usize, const M: usize>(
    name: &str,
    f: impl Fn(SVector<Expr, N>) -> SVector<Expr, M>,
) -> String {
